...
async fn test_manually_backup_all() {
    ...
    // check recovering remote path_secret
    let rebackuped_remote_ids = get_remote_ids(env::var("MY_ROSTER_IDX").unwrap().to_string());
    assert_eq!(rebackuped_remote_ids, remote_ids);
}
